#include "ASTConditional.cpp"
#include "Optimizer.h"

ASTConditional::ASTConditional(unsigned int t_referenceLine, ASTBool * const t_pConditional, 
    ASTNode * const t_pTrue, ASTNode * const t_pFalse) : ASTNode(t_referenceLine)
...
constexpr unsigned int ELSE_CONDITIONAL_LENGTH = MIN_CONDITIONAL_LENGTH + 5;
constexpr unsigned int GOTO_SKIP_LENGTH = CONDITIONAL_LENGTH + 6 + RESERVED_GOTO_LENGTH;

std::string ASTConditional::process() const override
{
    std::string rStr;
